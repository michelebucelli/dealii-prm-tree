...
export default class DealiiPrmTreeView {
  ...
  constructor() {
    ...
    this.filterInput.setAttribute("placeholder", "Filter...");
    this.filterInput.classList.add("input-text", "native-key-bindings");
    this.filterInput.oninput = () => {
      this.tree.filter = this.filterInput.value;
    ...
    this.rootElement.appendChild(this.filterInput);
  ...
  update(tree) {
    let filterString = tree.filter ? tree.filter.trim() : "";
    this.filterInput.value = tree.filter ? tree.filter : "";
    ...
    let processSection =
        function(root, name, section, path, tooltips, filterString) {
      if (filterString == "") {
        ...
        if (name.toLowerCase().includes(filterString.toLowerCase())) {
          let sectionNode = document.createElement("li");
          sectionNode.classList.add("list-item");
          sectionNode.textContent = path;
          sectionNode.targetLine = section.line;
          sectionNode.onclick = function(e) {
            let editor;
            if (editor = atom.workspace.getActiveTextEditor()) {
              editor.setCursorBufferPosition([ this.targetLine, 0 ]);
              if (atom.config.get("dealii-prm-tree.focusEditorOnClick"))
          ...
          if (atom.config.get("dealii-prm-tree.pathTooltipsOnHover"))
            sectionNode.setAttribute("title", path);
        ...
        if (section.params) {
          for (let i = 0; i < section.params.length; ++i)
            if (section.params[i].name.toLowerCase().includes(
              ...
              let paramName = document.createElement("span");
              paramName.textContent = path + "/" + section.params[i].name;
              if (atom.config.get("dealii-prm-tree.pathTooltipsOnHover"))
                paramNode.setAttribute("title",
                                       path + "/" + section.params[i].name);
              paramName.id = "name";
          ...
          if (section.subsections)
            for (let [key, value] of section.subsections)
              processSection(root, key, value, path + "/" + key, tooltips,
                             filterString);
        }
      }
    }
    ...
    if (tree.subsections)
      for (let [key, value] of tree.subsections)
        processSection(root, key, value, key, this.tooltips, filterString);
