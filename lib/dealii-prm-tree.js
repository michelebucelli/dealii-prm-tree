  ...
  activate(state) {
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'dealii-prm-tree:toggle' : () => {
        // If already active, we hide it.
        if (atom.workspace.getActivePane().getActiveItem().getURI() ==
            'atom://dealii-prm-tree-view')
          atom.workspace.hide('atom://dealii-prm-tree-view');
        // Oterwise we show it and focus the filter box.
        else
          atom.workspace.open('atom://dealii-prm-tree-view').then(() => {
            this.update();
            this.prmTreeView.filterInput.focus();
          });
      }
    }));
